...
import 'package:halo/widget/extended_image/src/extended_image.dart';
import 'package:halo/widget/extended_image/src/extended_image_utils.dart';
import 'package:halo/widget/extended_image/src/gesture/extended_image_gesture_page_view.dart';
import 'package:halo/widget/extended_image/src/gesture/extended_image_gesture_utils.dart';
import 'package:halo/widget/extended_image/src/image/extended_raw_image.dart';
...
class ExtendedImageGesture extends StatefulWidget {
  ...
  ExtendedImageGesture(
      this.extendedImage, this.extendedImageState, this.extendedImagePageViewState);
  @override
...
class _ExtendedImageGestureState extends State<ExtendedImageGesture>
  ...
  void _handleScaleUpdate(ScaleUpdateDetails details) {
    double scale = _clampScale((_startingScale * details.scale * _gestureConfig.speed),
    ...
    var offset =
        ((details.scale == 1.0 ? details.focalPoint : _startingOffset) - _normalizedOffset * scale);
  ...
  void _handleScaleEnd(ScaleEndDetails details) {
    ...
    if (_gestureDetails.totalScale > _gestureConfig.maxScale) {
      final double velocity =
          (_gestureDetails.totalScale - _gestureConfig.maxScale) / _gestureConfig.maxScale;
    ...
    if (_gestureDetails.totalScale < _gestureConfig.minScale) {
      final double velocity =
          (_gestureConfig.minScale - _gestureDetails.totalScale) / _gestureConfig.minScale;
    ...
    if (_gestureDetails.gestureState == GestureState.pan) {
      ...
      if (magnitude >= minMagnitude) {
        final Offset direction =
            details.velocity.pixelsPerSecond / magnitude * _gestureConfig.inertialSpeed;
...
//    if (_gestureConfig.inPageView) {
  ...
  void handleDoubleTap({double scale, Offset doubleTapPosition}) {
    ...
    _handleScaleUpdate(
        ScaleUpdateDetails(focalPoint: doubleTapPosition, scale: scale / _startingScale));
  }
