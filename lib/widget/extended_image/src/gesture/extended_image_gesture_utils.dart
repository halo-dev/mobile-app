  ...
  Offset _getCenter(Rect destinationRect) {
    ...
    if (totalScale > 1.0) {
      if (_computeHorizontalBoundary && _computeVerticalBoundary) {
        ...
        return Offset(destinationRect.center.dx * totalScale, destinationRect.center.dy) +
            Offset(offset.dx, 0.0);
      } else if (_computeVerticalBoundary) {
        //only scale Vertical
        return Offset(destinationRect.center.dx, destinationRect.center.dy * totalScale) +
  ...
  Rect _innerCalculateFinalDestinationRect(Rect layoutRect, Rect destinationRect) {
    ...
    if (_computeHorizontalBoundary) {
      ...
      if (result.right <= layoutRect.right) {
        result =
            Rect.fromLTWH(layoutRect.right - result.width, result.top, result.width, result.height);
    ...
    if (_computeVerticalBoundary) {
      ...
      if (result.bottom <= layoutRect.bottom) {
        result = Rect.fromLTWH(
            result.left, layoutRect.bottom - result.height, result.width, result.height);
